...
Reglas:
- Se cancela un Turno específico (actividad + fecha + hora).
- Se cancelan las reservas activas del turno (confirmada / en espera / invitado).
- Compensación:
  - PAGADO o abonado mensual -> crédito del deporte.
  - SEÑADO -> reintegro de seña (simulado) + mail.
  - PENDIENTE -> sin cargo + mail informativo.
- Importante: no se reutiliza Reserva.cancelar() para NO disparar invitaciones de lista
  de espera (la clase queda cancelada por el club).
"""
...
def validar_fecha_hora_futura(fecha: date, hora: int) -> None:
    if inicio_turno(fecha, hora) <= timezone.now():
        raise ValidationError(_("La fecha y el horario deben ser posteriores al momento actual."))


def buscar_turno_clase(actividad, fecha: date, hora: int) -> Turno | None:
    try:
        return (
            Turno.objects.select_related("actividad")
...
def reservas_activas_turno(turno: Turno) -> list[Reserva]:
    return list(
        turno.reservas.filter(estado__in=_estados_activos())
...
def clasificar_compensacion(reserva: Reserva) -> tuple[TipoCompensacion, str]:
    ...
    if reserva.estado_pago == Reserva.EstadoPago.PAGADO or reserva.es_abonado_mensual:
        return (
...
def ejecutar_cancelacion_clase(turno: Turno) -> list[ImpactoReserva]:
    ...
    for impacto in impactos:
        ...
        elif impacto.compensacion == TipoCompensacion.REEMBOLSO_SENA:
